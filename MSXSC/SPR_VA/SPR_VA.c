...
unsigned short set_sprite_pattern(char *loadfil, unsigned short adr)
{
	...
	for(spr_no2 = 0; spr_no2 < 4; ++spr_no2){
		spr_y = spr_no2 * (SPR_Y) * 8;
		for(count = 0; count < LINE; ++count){
			...
			for(count2 = 0; count2 < (WIDTH / 2 / 2); ++count2){
				...
				/* 横を2倍する */
				index = adr + ((spr_no) * SPR_SIZE) + spr_x + (spr_y * (SPR_X / 2)) ;
				for(m = 0; m < 2; ++m){
					tvram[index + m * 4 + 0] = 
...
void set_sprite_locate(unsigned short pat_no, unsigned short x, unsigned short y, unsigned short chr_no){
	pat_no *= 8;
	spr_atr[pat_no + 0] = y % 256;				/* Y */
	spr_atr[pat_no + 1] = (SPR_Y / 4 - 1) * 4 | 0x02 | ((y / 256) & 0x01);
	spr_atr[pat_no + 2] = x % 256;				/* X */
	spr_atr[pat_no + 3] = (SPR_X / 8 - 1) * 8 | ((x / 256) & 0x03);
	/* データアドレス(TSP) */
	spr_atr[pat_no + 4] = ((SPR_PAT_ADR + SPR_SIZE * chr_no) / 2) % 256;
	spr_atr[pat_no + 5] = ((SPR_PAT_ADR + SPR_SIZE * chr_no) / 2) / 256;
}
...
//	clear(3);
	...
	for(i = 0; i < 32; ++i){
		set_sprite_locate(i, (i % 16) * SPR_X, (i / 16) * SPR_Y, i);
		index = (i + 1) * 8;
		spr_atr[index + 6] = 0;
		spr_atr[index + 7] = 0;
	}
	...
	if(set_sprite_pattern("ELMAPCHR.SC5", SPR_PAT_ADR)){
		end();
		exit(1);
	}
...
/* ESCで抜けるまでループ*/
	while(((k9 = inportb(0x09)) & 0x80)){ /* ESC */
		...
		set_sprite_locate(spr_no, x, y, 	chr_tbl[dir * 2 + dir2][0] / 2);
